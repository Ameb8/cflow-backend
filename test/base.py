...
class TestSetup(APITestCase):
    def setUp(self):
        ...
        self.root_folder = Folder.objects.create(folder_name='root', user=self.user, parent=None)

        # Create subfolder
        self.subfolder = Folder.objects.create(folder_name='subfolder', user=self.user, parent=self.root_folder)

        # Create files
        File.objects.create(file_name='file_main', folder=self.root_folder, extension='c', file_content=MAIN_CONTENT)
